...
export class UserProfilePage extends Component {
  ...
  async loadUserProfileData() {
    const { getToken } = await import('../../../js/clients/token-client.js');
    const token = getToken();
    if (token) {
      const { getUserProfile } = await import('../../../js/clients/users-client.js');
      try {
        const userProfile = await getUserProfile(token);

        // Pass data to components via attributes
        this.querySelector('user-profile-header').setAttribute('data', JSON.stringify(userProfile));
        this.querySelector('user-profile-content').setAttribute('data', JSON.stringify(userProfile));
        this.querySelector('user-profile-stats-cards').setAttribute('data', JSON.stringify(userProfile));
