...
export class TournamentContent extends Component {
    ...
    displayRanks() {
        ...
        const sortedPlayers = Object.entries(this.playerWins)
            .sort((a, b) => b[1] - a[1]);
    
        // Generate the HTML for the ranks table
        let rankHtml = sortedPlayers.map(([player, wins], index) => `
            <tr>
                <td>${index + 1}</td>
                <td>${player}</td>
                <td>${wins}</td>
    ...
    endMatch(winner, player1, player2) {
        ...
        this.showOverlay(`🏆 ${winner} wins! ❌ ${loser} is eliminated!`, () => {
    ...
    startGame(player1, player2) {
        ...
        this.engine = new Engine(this, (winner) => this.endMatch(winner, player1, player2));
...
customElements.define("tournament-content-component", TournamentContent);
