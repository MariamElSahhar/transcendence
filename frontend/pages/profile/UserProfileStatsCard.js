...
export class UserProfileStatsCard extends Component {
  ...
  // Main render function to decide whether to render placeholder or values
  render() {
    ...
    this.footerValue = this.getAttribute('footer-value') || '0';
    this.valueConcatenate = this.getAttribute('value-concatenate') || '';
    this.footerTitle = this.getAttribute('footer-title');

    // Call the function based on placeholder attribute
    return this.placeholder === 'true' ? this.renderPlaceholder() : this.renderWithValues();
  }
    ...
    return `
      <div class="card placeholder-glow">
        <div class="card-body">
          <div class="row">
            <div class="col">
              <span class="h6 placeholder col-5">_</span>
              <span class="h3 placeholder col-3">_</span>
            </div>
            <div class="col-auto">
              <div class="icon icon-shape ${this.iconBackground}"></div>
            </div>
          </div>
          <div class="text-sm mt-2">
            <span class="placeholder col-8">_</span>
          </div>
        </div>
      </div>
    `;
    ...
    return `
      <div class="card">
        <div class="card-body">
          <div class="row">
            <div class="col">
              <span class="h6 text-muted text-sm">${this.title}</span>
              <span class="h3">${this.value}</span>
            </div>
            <div class="col-auto">
              <div class="icon icon-shape text-white rounded-circle ${this.iconBackground}">
                <i class="bi ${this.icon}"></i>
              </div>
            </div>
          </div>
          <div class="d-flex align-items-center text-sm">
            <span class="text-muted">${this.footerTitle}</span>
          </div>
        </div>
      </div>
    `;
  ...
  // Method to load values dynamically after rendering
  loadValues(mainValue, footerValue, valueConcatenate = '') {
    this.setAttribute('value', mainValue + valueConcatenate);
    this.setAttribute('footer-value', footerValue + valueConcatenate);
    this.setAttribute('placeholder', 'false');
    this.innerHTML = this.renderWithValues();
  }
}

customElements.define('user-profile-stats-card-component', UserProfileStatsCard);
...
//                     </div>
//                     <div class="col-auto">
//                         <div class="icon icon-shape text-lg rounded-circle ${placeholderClass}"></div>
//                     </div>
//                 </div>
...
//                 <div class="row">
//                     <div class="col">
//                         <span class="h6 font-semibold text-muted text-sm d-block mb-2">${this.title}</span>
...
//                     </div>
//                 </div>
//                 <div class="d-flex align-items-center mt-2 mb-0 text-sm">
//                     <span class="badge ${this.footerValueBrackground} ${this.footerValueColor} me-2">
//                         <i class="bi ${this.footerArrow} me-1"></i>${this.footerValue}
//                     </span>
//                     <span class="text-nowrap text-xs text-muted">${this.footerTitle}</span>
//                 </div>
//             </div>
//         </div>
//     `);
...
// customElements.define('user-profile-stats-card', UserProfileStatsCard);
