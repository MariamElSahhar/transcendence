...
export class UserProfileContent extends Component {
	...
	async fetchStatistics(userId) {
		...
		const month = this.convertDatesToIsoString(
			this.getDatesFromLastMonth()
		);
	...
	async addUsernameInMatchHistory(matchHistory) {
		const opponentsIds = matchHistory["history"].map((match) =>
			parseInt(match["opponent_id"])
		);
		try {
			const { response, body } =
