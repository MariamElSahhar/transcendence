...
import { Segment2 } from "../../../local/Scene/Segment2.js"
export class Paddle {
  ...
  #moveAIPaddle(ballPosition, ballVelocity) {
    ...
    const anticipatedPositionY = ballPosition.y + (ballVelocity?.y || 0) * anticipationFactor;
    const distanceToBall = anticipatedPositionY - this.#threeJSGroup.position.y;
    ...
    if (Math.abs(distanceToBall) > movementThreshold) {
        ...
        const newYPosition = this.#threeJSGroup.position.y + (distanceToBall > 0 ? aiSpeed : -aiSpeed);
  ...
  setDirection(direction) {
    this.#movement.y = direction === 'up' ? this.#moveSpeed : direction === 'down' ? -this.#moveSpeed : 0.;
  }
  ...
  #addPaddleToGroup(color) {
    this.#paddleObject = new THREE.Mesh(
      new THREE.BoxGeometry(this.#paddleSize.x, this.#paddleSize.y, this.#paddleSize.z),
  ...
  #setCollisionSegments() {
    const xLeft = this.#playerPosition.x + this.#threeJSGroup.position.x - this.#paddleSize.x * 0.5;
    const xRight = this.#playerPosition.x + this.#threeJSGroup.position.x + this.#paddleSize.x * 0.5;

    const yTop = this.#playerPosition.y + this.#threeJSGroup.position.y + this.#paddleSize.y * 0.5;
    const yBottom = this.#playerPosition.y + this.#threeJSGroup.position.y - this.#paddleSize.y * 0.5;
    ...
    this.#frontCollisionSegment = this.#paddleIsOnTheRight ? new Segment2(bottomLeft, topLeft) : new Segment2(bottomRight, topRight);
  }
