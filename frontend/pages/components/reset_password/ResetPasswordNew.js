...
export class ResetPasswordNew extends Component {
				...
				<div class="card-body m-2">
					...
					<form id="form">
						...
						<div class="d-flex justify-content-center w-100">
							<button id="confirm-btn" type="submit" class="btn btn-primary w-100" disabled>Change password</button>
						</div>
					</form>
				</div>
			</div>
	...
	postRender() {
		...
		super.addComponentEventListener(
			...
			"input",
			this.#confirmPasswordHandler.bind(this)
		);
		super.addComponentEventListener(
	...
	#setInputConfirmPasswordValidity(
		validity,
	...
	#toggleConfirmPasswordVisibility() {
		...
		this.confirmPassword.type = this.confirmPasswordHidden
			? "password"
			: "text";
	...
	async #confirmBtnHandler() {
		...
		try {
			const { response, body } =
				await userManagementClient.changePassword(
					this.email,
					this.code,
					this.password.value
				);
