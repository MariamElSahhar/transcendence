...
export class ResetPasswordCode extends Component {
	...
	postRender() {
		...
		for (const input of this.inputs) {
			super.addComponentEventListener(input, 'keydown', this.#handleInputChange);
	...
	async #sendCode() {
		...
		try {
			const { response, body } = await userManagementClient.checkResetPasswordCode(this.email, this.code);
			if (response.ok) {
	...
	#notifyCodeVerified() {
		...
		const codeEvent = new CustomEvent('codeVerified', { detail: { code: this.code } });
