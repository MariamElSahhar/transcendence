...
export class UserProfileContent extends Component {
  ...
  updateComponents(userId, username, statistics) {
    this.userProfileHeader = this.querySelector('user-profile-header-component');
    this.userProfileHeader.loadUserProfile(username, userId);

    this.statisticsCards = this.querySelector('user-profile-stats-cards-component');
    this.statisticsCards.loadStatistics(statistics.general, statistics.progress);

    this.chartsCards = this.querySelector('user-profile-charts-cards-component');
    ...
    this.userProfileMatchList = this.querySelector('user-profile-match-list-component');
    this.userProfileMatchList.loadMatchHistory(userId, statistics.matchHistory);
  }
  ...
  async fetchStatistics(userId) {
    ...
    const [
      ...
      this.fetchData('getMatchesPlayedGraph', userId, week.startDate, week.endDate),
      this.fetchData('getMatchHistory', userId, 1),
    ]);

    if ([general, progress, eloGraph, winRateGraph, matchesPlayedGraph, matchHistory].includes(false)) {
      return false;
    }
    if (!await this.addUsernameInMatchHistory(matchHistory)) {
      return false;
    }
    return { general, progress, eloGraph, winRateGraph, matchesPlayedGraph, matchHistory };
  }
  ...
  async getUserId(username) {
    try {
      const { response, body } = await userManagementClient.getUserByUsernameInCache(username);
      if (response.ok) {
        return body.id;
  ...
  convertDatesToIsoString(dates) {
    return { startDate: dates.startDate.toISOString(), endDate: dates.endDate.toISOString() };
  }
}

// Register the custom element
customElements.define('user-profile-content', UserProfileContent);
